...
<%
	Biglietto product = (Biglietto) request.getAttribute("biglietto");
...
<link href="HomePage/CSS/Biglietto.css" rel="stylesheet">
...
<h2>Catalogo Biglietti</h2>
	<%if(uadmin!=null){ %>
	<form method="post">
		<input id="b3" type="submit" name="insert" value="INSERISCI NUOVO ARTICOLO"/><br><br>
	</form>
	
	<%if(request.getParameter("insert")!=null){ %>  <!-- Clicca per inserire un articolo -->
		<div id="mod">
							<fieldset>
								<legend><b>INSERISCI ARTICOLO</b></legend>
								<form method="post" action="productbig?azione=inserisci">
	...
	<table>
		...
		Collection<?> biglietto = (Collection<?>) model.doRetrieveAll("biglietto");
		Iterator<?> it = biglietto.iterator();
		while (it.hasNext()) {
			 Biglietto bean = (Biglietto) it.next();
		%>
		<tr>
			...
			<td>
			<%if (uadmin!=null){ %>
			<button id="d" onclick="window.location='productbig?azione=cancella&id=<%=bean.getCodice()%>'">Cancella</button>
			<form method="post" action="productbig?azione=modifica&id=<%=bean.getCodice()%>">
					<input id="d" type="submit" name="mod" value="Modifica"/>
			</form>			
			<%} %>
			
			<form method="post" action="productbig?azione=dettagli&id=<%=bean.getCodice()%>">
				<input id="d" type="submit" name="dettagli" value="Dettagli">
			</form>
			<%if(uadmin==null){ %>
				<form method="post" action="productbig?azione=aggiungialCarrello&id=<%=bean.getCodice()%>">
...
<%if(request.getParameter("dettagli")!=null && product!=null){%>
	...
	<%if(request.getParameter("mod")!=null && product!=null){%>
		<div id="mod">
							...
							<form method="post" action="productbig?azione=confermamodifica&id=<%=product.getCodice()%>">
								<label for="nome">Nome:</label><br>
								<input type="text" name="Nome" required><br><br>
