...
@WebServlet("/productga")
public class ProductControlGad extends HttpServlet {
	private static final long serialVersionUID = 1L;
   
	static ProductModel model=new ProductModelDM();

	
    public ProductControlGad() {
        super();
    }
		...
		try{
			if(azione!=null){
				if(azione.equalsIgnoreCase("aggiungialCarrello")){     
					...
					String id=request.getParameter("id");
					request.removeAttribute("gadget");
					request.setAttribute("gadget", model.doRetrieveByKey(id));
					...
					String id=request.getParameter("id");
					model.doDelete(id);
					response.sendRedirect("/PROGETTO_CIRCOLO/Gadget.jsp");
					...
					insertSQL="INSERT INTO GADGET (Codice) VALUES(?)";
